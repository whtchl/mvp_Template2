...
import com.tchl.server3.http.utils.NetworkUtil;
...
logging.setLevel(HttpLoggingInterceptor.Level.BODY);*/
    ...
    private static class LoggingInterceptor implements Interceptor {
        ...
        public Response intercept(Chain chain) throws IOException {
            ...
            JUtils.Log( String.format(Locale.CHINA, "Header:  Sending request %s on %s%n%s",
            ...
            JUtils.Log(String.format(Locale.CHINA, "Header: Received response for %s in %.1fms%n%s",
                    response.request().url(),(t2 - t1) / 1e6d, response.headers()));
            /*ResponseBody responseBody = response.peekBody(1024 * 1024*9);

            JUtils.Log(String.format(Locale.CHINA, "Received response for %s %n返回json:【%s】  in %.1fms%n%s",
