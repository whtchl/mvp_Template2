...
import com.lifeng.beam.expansion.BeamBasePresenter;
...
public class BeamListActivityPresenter<T extends BeamListActivity,M> extends BeamBasePresenter<T>
    ...
    Subscriber<List<M>> mRefreshSubscriber = new Subscriber<List<M>>() {
        ...
        public void onCompleted() {
            JUtils.Log("mRefreshSubscriber onCompleted ");
        }

        @Override
        public void onError(Throwable e) {
            JUtils.Log("mRefreshSubscriber onError ");
            inited = true;
        ...
        public void onNext(List<M> ms) {
            JUtils.Log("mRefreshSubscriber onNext ");
            inited = true;
            //用新的替换，表示清空数据
        ...
        mAdapterSubscription = mDataSubject.subscribe(new Subscriber<List<M>>() {
            ...
            public void onError(Throwable e) {
                ...
                if (mAdapter!=null&&mAdapter.getCount()==0){
                    ...
                    JUtils.Log("没有数据的时候出错，就显示全屏的错误提示");
                    getView().showError(e);
                }else {
                    //有数据时的出错，在最后一条显示错误提示
                    JUtils.Log("/有数据时的出错，在最后一条显示错误提示");
