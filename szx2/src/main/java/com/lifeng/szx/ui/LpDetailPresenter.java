...
public class LpDetailPresenter extends Presenter<LpDetailActivity> {

    public void pQueryLpDetail(String id,String third,String mchcode){
        if (!CheckNetIsOk.isNetOk(getView())) {
            return;
        }
        getView().getExpansion().showProgressDialog(getView().getString(R.string.waiting));

        SzxModel.getInstance().queryLpDetail(id, third, mchcode)
                ...
                .subscribe(new ServiceResponseSzx<SzxTransResponse>() {
                    ...
                    protected void _onNext(SzxTransResponse lpResponse) {
                        ...
                        getView().UpdateDetail(lpResponse);
